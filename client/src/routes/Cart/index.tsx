...
function CartPage() {
    ...
    return <div className="m-24">
        ...
        <div className="block md:flex ">
            {cartItems.length ? <CartItems items={cartItems} /> : <div role="alert" className="alert alert-error">
                <CrossIcon />
...
function CartItems({ items }: any) {
    ...
    return <div className="relative overflow-x-auto shadow-md sm:rounded-lg w-1/2">
        <h1 className="font-extrabold underline text-2xl  text-center">Current Items in the cart</h1>
                ...
                {items.map((item: any, index: number) => {
                    return <tr>
                        ...
                        <td className="px-6 py-4 text-right">
                            <button className="btn btn-error btn-outline" onClick={() => dispatch(removeFromCart(item.id))}><Trash2 /></button>
                        </td>
                    </tr>
                })}
            </tbody>
...
function CheckoutScreen({ items }: any) {
    ...
    return <div className="w-1/2  flex justify-end">
        <div className="border w-fit p-8 rounded-xl text-center flex flex-col gap-4">
            ...
            <Link to={'/checkout'}><button className="btn w-full btn-outline btn-primary">Checkout</button></Link>
        </div>
    </div>;
}
